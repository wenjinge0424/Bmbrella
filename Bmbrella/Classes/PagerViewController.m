...
- (void) launchPageViewController {
    ...
    pageDescriptions = @[LOCALIZATION(@"title_one"), LOCALIZATION(@"title_two"), LOCALIZATION(@"title_three")];
    
    pageImages = @[@"bg_page_one",
                        @"bg_page_two",
                        @"bg_page_three",
                        @"groups"];
...
//    }
    ...
    // goto Next screen after last page
    if (index == [pageTitles count]) {
        PageItemViewController *pageItemVC = [PageItemViewController new];
        pageItemVC.pageIndex = [pageTitles count];
        return pageItemVC;
    }
    return [self viewControllerAtIndex:index];
}
...
// goto Next screen after last page
- (void)pageViewController:(UIPageViewController *)pageViewController willTransitionToViewControllers:(NSArray<PageItemViewController *> *)pendingViewControllers {
    PageItemViewController *viewController = [pendingViewControllers objectAtIndex:0];
    
    if ([viewController pageIndex] == pageTitles.count){
        [self gotoLoginScreen];
    }
}
@end
